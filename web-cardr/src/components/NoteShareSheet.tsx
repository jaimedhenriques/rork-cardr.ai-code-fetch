...
import { X, Link2, Mail, FileText, Check, Copy, Loader2, Users, FileSpreadsheet, ExternalLink, FileDown, FileType } from "lucide-react";
...
const NoteShareSheet = ({ note, open, onClose }: NoteShareSheetProps) => {
  ...
  /** Renders the AI-polished markdown notes as simple HTML for PDF/Word exports. */
  const polishedHtml = (md: string) =>
    md.split("\n").map(line => {
      const t = line.trim();
      if (!t) return "";
  ...
  /** Full HTML document rendering of the note — shared by the PDF and Word exports. */
  const buildExportHtml = () => {
    ...
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${note.title || "Meeting Notes"}</title>
...
${note.enhanced_notes ? section("Polished Notes", polishedHtml(note.enhanced_notes)) : ""}
...
<p class="meta" style="margin-top:32px;text-align:center;border-top:1px solid #e5e7eb;padding-top:12px">Generated by Cardr · ${dateStr}</p>
  ...
  const safeFilename = () =>
    `${(note.title || "meeting-notes").replace(/[^a-z0-9]/gi, "-")}-${format(parseISO(note.created_at), "yyyy-MM-dd")}`;
  ...
  /** Word-compatible export — HTML saved as .doc opens directly in Word/Pages/Google Docs. */
  const downloadWord = () => {
  ...
  /** Markdown export — drops straight into Notion, Obsidian, or any docs tool. */
        ...
        >
          ...
          <div className="space-y-2">
            {[
              ...
              { fn: downloadWord, icon: FileType, label: "Export as Word", desc: "Opens in Word, Pages & Google Docs", color: "text-primary", bg: "bg-primary-light" },
              { fn: downloadMarkdown, icon: FileDown, label: "Export as Markdown", desc: "For Notion, Obsidian & docs tools", color: "text-accent", bg: "bg-primary-light" },
